...
def m2o_to_external(field, binding=None):
    ...
    def modifier(self, record, to_attr):
        ...
        if column.type != "many2one":
            raise ValueError(
                "The column %s should be a Many2one, got %s"
                % (field, type(column))
            )
...
MappingDefinition = namedtuple(
    "MappingDefinition", ["changed_by", "only_create"]
)
...
class Mapper(AbstractComponent):
            ...
            if hasattr(base, "_map_methods"):
                ...
                for attr_name in dir(base):
                    ...
                    definition = MappingDefinition(
                        mapping_changed_by, has_only_create
                    )
                    map_methods[attr_name] = definition
    ...
    def _get_map_child_component(self, model_name):
        try:
            ...
            assert (
                self._map_child_fallback is not None
    ...
    def changed_by_fields(self):
        ...
        for _, method_def in self._map_methods.items():
    ...
    def _apply_with_options(self, map_record):
        ...
        _logger.debug(
            "converting record %s to model %s", map_record.source, self.model
        )
